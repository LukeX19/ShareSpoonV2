...
import { IRecipe, IRecipeRequest } from '../../interfaces/RecipeInterface';
import { useFormik } from 'formik';
import * as Yup from 'yup';
import { getRecipeById, updateRecipe } from '../../services/RecipeService';
import { uploadFile } from '../../services/FileService';
import { IRecipeIngredient } from '../../interfaces/RecipeIngredientInterface';
    ...
    const fetchRecipe = async () => {
      if (recipeId) {
        try {
          ...
          if (user && response && response.userId !== user.id) {
            navigate('/forbidden');
            return;
          }
          setRecipe(response);
          ...
          formik.setValues({
            ...
            ingredients: response.recipeIngredients.map((ingredient: IRecipeIngredient) => ({
              id: ingredient.id,
            ...
            tags: response.recipeTags.map((tag: ITag) => ({
              id: tag.id,
              name: tag.name,
              type: tag.type
            })),
          ...
          const [hrs, mins] = response.estimatedTime.split(':').map(Number);
          setHours(hrs.toString().padStart(2, '0'));
          setMinutes(mins.toString().padStart(2, '0'));
          setDifficulty(response.difficulty);
          setInstructions(response.description);
          setSelectedIngredients(response.recipeIngredients.map((ingredient: IRecipeIngredient) => ({
            id: ingredient.id,
            name: ingredient.name,
            quantity: ingredient.quantity,
            quantityType: ingredient.quantityType,
          })));
          setSelectedTags(response.recipeTags.map((tag: ITag) => ({
            id: tag.id,
            name: tag.name,
            type: tag.type
          })));
  ...
  const formik = useFormik<IRecipeRequest>({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const response = await updateRecipe(Number(recipeId), formData);
